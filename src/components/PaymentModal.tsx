...
export default function PaymentModal({ onClose }: { onClose: () => void }) {
  const { totals, checkout, method, branch, settings, paymentMethods } = useStore();
  ...
  useEffect(() => {
    if (done && settings.receiptAction === "direct") window.print();
  }, [done, settings.receiptAction]);

  const quickCash = [due, Math.ceil(due), Math.ceil(due / 5) * 5, Math.ceil(due / 10) * 10]
      ...
      <Shell onClose={onClose}>
        <div className="px-7 pb-7 pt-8">
          ...
          <p className="mt-1 text-center text-sm text-muted">
            {t("pay.orderRef", { number: done.number })} · {dateTime(done.createdAt, locale)}
          </p>

          <div
            className="mx-auto mt-6 rounded-2xl border border-line bg-surface p-5 font-mono text-[13px]"
            style={{ maxWidth: settings.receiptPrinter.paperWidth === "58" ? 260 : 340 }}
          >
            {settings.printLogo && (
              <Image
                src="/logo-mark.png"
            ...
            {done.discount > 0 && <Row label={t("common.discount")} value={`-${currency(done.discount)}`} />}
            <Row label={t("common.total")} value={currency(done.total)} bold />
            <Row label={t("pay.paidWith", { method: label })} value={currency(done.paid)} />
  ...
  return (
    <Shell onClose={onClose}>
      <div className="flex items-center justify-between border-b border-line px-6 py-5">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">{t("pay.title")}</h2>
          <p className="text-xs text-muted">{t("pay.payingWith", { method: label })}</p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="px-6 py-6">
        <p className="text-xs font-medium uppercase tracking-wider text-muted">{t("pay.amountDue")}</p>
        <p className="mt-1 text-4xl font-semibold tracking-tight tabular-nums">{currency(due)}</p>
        ...
        {isCash ? (
          <div className="mt-6">
            ...
            <div className="mt-4 flex items-center justify-between rounded-xl bg-surface px-4 py-3.5 text-sm">
              ...
              <span className="text-base font-semibold tabular-nums">{currency(change)}</span>
            </div>
          </div>
        ) : (
      ...
      <div className="border-t border-line px-6 py-5">
        ...
        >
          {canPay ? t("pay.charge", { amount: currency(due) }) : t("pay.insufficient")}
        </button>
      </div>
    </Shell>
  );
